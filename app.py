...
hashed_passwor = generate_password_hash('admin123')
cur.execute("SELECT * FROM users WHERE email = ?", [admin_email])
if not cur.fetchone():
    cur.execute("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)",
                ['Администратор', admin_email, hashed_passwor, 'admin'])
    conn.commit()
...
def add_user(name, email, password):
    hashed_passwor = generate_password_hash(password)
    cur.execute('INSERT INTO users(name, email, password, last_login) VALUES (?, ?, ?, ?)', 
                [name, email, hashed_passwor, datetime.now()])
    conn.commit()
